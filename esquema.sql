...
--
-- Atualizado em 06/09/2026 (tarde, ecrã "Propriedades e Unidades"
-- da GUI): `propriedades` e `unidades` ganharam
-- `desativado_por_id`/`data_desativacao` — só preenchidos quando
-- uma desativação é FORÇADA (dependências ativas ignoradas
-- conscientemente), para saber quem autorizou. Numa base já
-- existente, ver a nota junto às duas tabelas (ALTER TABLE, não
-- este CREATE TABLE).
-- ============================================================
...
-- propriedades
--
-- NOTA (06/09/2026): `desativado_por_id`/`data_desativacao` são
-- novos. Numa base já existente, NÃO uses este CREATE TABLE —
-- corre antes, no teu MySQL real:
--
--   ALTER TABLE propriedades
--       ADD COLUMN desativado_por_id VARCHAR(10) NULL AFTER ativo,
--       ADD COLUMN data_desativacao DATE NULL
--           AFTER desativado_por_id,
--       ADD CONSTRAINT fk_propriedades_desativado_por
--           FOREIGN KEY (desativado_por_id) REFERENCES
--           responsaveis(id);
--
-- Só ficam preenchidos quando `propriedades.desativar(...,
-- forcar=True, responsavel_id=...)` é usado com dependências
-- ativas — uma desativação normal (sem forçar) deixa os dois a
-- NULL, e `reativar` volta a limpá-los.
-- ------------------------------------------------------------
...
-- unidades
--
-- NOTA (06/09/2026): mesma coisa de `propriedades`, acima —
...
--       ADD COLUMN data_desativacao DATE NULL
--           AFTER desativado_por_id,
--       ADD CONSTRAINT fk_unidades_desativado_por
--           FOREIGN KEY (desativado_por_id) REFERENCES
--           responsaveis(id);
-- ------------------------------------------------------------
